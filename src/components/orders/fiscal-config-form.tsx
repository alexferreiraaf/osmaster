...
type FormIconType = 'user' | 'shield' | 'hard-drive';
...
export function FiscalConfigForm({ employees }: { employees: Employee[] }) {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    startTransition(async () => {
       try {
            ...
            const dataToSave = {
                ...rest,
                orderNow: 'Não' as const,
                mobile: 'Não' as const,
                ifoodIntegration: 'Não' as const,
            ...
            await createOrder(dataToSave, user, certificate, undefined);

            toast({ title: 'Sucesso!', description: 'Nova ordem de serviço (Configuração Fiscal) criada.' });
            router.push('/orders');
        } catch (error) {
            console.error(error);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <h2 className="text-2xl font-bold tracking-tight">Nova OS: Configuração Fiscal</h2>
      ...
      <div className="flex gap-3 pt-4 border-t">
        ...
        <Button type="button" variant="outline" size="lg" onClick={() => router.push('/orders/new')}>
          Cancelar
        </Button>
      </div>
    </form>
